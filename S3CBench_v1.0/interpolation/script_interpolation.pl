...
my $in_file;
my $golden_file;
my $source_file="filter_interp.cpp";
my $tr_type;
...
system("make","$tr_type");}
if(!$golden_file){
system("./interpolation.exe -infile $in_file");
}
else{
system("./interpolation.exe -infile $in_file -golden $golden_file");}
system("gcov","-r","$source_file");
#system("gedit","$source_file.gcov");
system("lcov","--capture","--directory",".","--output-file","interp.info","--no-external");
system("lcov","--remove","interp.info","filter_interp.h","--output","interp_final.info");
system("genhtml","interp_final.info","--output-directory",".");
...
{
  print "\nHELP MENU ";
  printf "\n\nCommand format is:\n\tperl script_interpolation.pl -infile <inputfile.txt> -goldfile <goldenfile.txt> -t <trojan_type>  \n\n";
  printf("\nOptions : ");
  ...
  printf "Sequential Without Memory           |            swom   \n"; 
