...
system("make","$tr_type");}
if(!$golden_file){
system("./fir.exe -infile $in_file");
}
else{
system("./fir.exe -infile $in_file -golden $golden_file");}
system("gcov","-r","$source_file");
#system("gedit","$source_file.gcov");
system("lcov","--capture","--directory",".","--output-file","fir.info","--no-external");
system("lcov","--remove","fir.info","fir.h","--output","fir_final.info");
system("genhtml","fir_final.info","--output-directory",".");
...
{
  print "\nHELP MENU ";
  printf "\n\nCommand format is:\n\tperl script_fir.pl -infile <inputfile.txt> -goldfile <goldenfile.txt> -t <trojan_type>  \n\n";
  printf("\nOptions : ");
