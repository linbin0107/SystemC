...
//22/07/2016        1.2            PolyU DARC Lab      Modified for Hardaware Trojan case 
//=======================================================================================
...
/*------------Sequential With Memoy Trojan-----------*/
#if defined(SWM) || defined(SWM_TRI)
            flag = (count1 > 2837) ? 1 : 0; //Flag is set to one when the count reaches specified value
            width.pre_data = (flag == 1) ? count3 : width.in_data;
#else
...
/*------------Sequential WithOut Memory Trojan-----------*/
#if defined(SWOM) || defined(SWOM_TRI)
            odata = (count1 % 1001 == 0) ? count2 : width.enc_data; // Triggers based on particular count value and the payload has no memory
#else
