...
my $in_file;
my $golden_file;
my $source_file="adpcm_encoder.cpp";
my $tr_type;
...
system("make","$tr_type");}
if(!$golden_file){
system("./adpcm_encoder.exe -infile $in_file");
}
else{
system("./adpcm_encoder.exe -infile $in_file -golden $golden_file");}
system("gcov","-r","$source_file");
system("lcov","--capture","--directory",".","--output-file","adpcm_encoder.info","--no-external");
system("lcov","--remove","adpcm_encoder.info","adpcm_encoder.h","--output","adpcm_encoder_final.info");
system("genhtml","adpcm_encoder_final.info","--output-directory",".");
...
{
  print "\nHELP MENU ";
  printf "\n\nCommand format is:\n\tperl script_adpcm.pl -infile <inputfile.txt> -golden <goldenfile.txt> -t <trojan_type>  \n\n";
  printf("\nOptions : ");
