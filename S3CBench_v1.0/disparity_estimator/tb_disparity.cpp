...
//---------------------------------------------------------------------------
//16/07/13        1.0      POLYU              Send/receive and compare outputs 
...
void test_disparity::initialize(){
  ...
  if (in_file){
    ...
    cout<<"Could not read bmp file "<<  infile <<endl;
    sc_stop();
  }
...
void test_disparity::compare_results(){
  ...
  // Variables
  int errors=0;
 FILE *ptr_tlv_out = NULL;
 FILE *ptr_tlv_out_3D = NULL;

 ptr_tlv_out= fopen("disparity_out.tlv", "wt");
 if(ptr_tlv_out == 0)
   exit(0);

 ptr_tlv_out_3D= fopen("disparity_out_3D.tlv", "wt");
 if(ptr_tlv_out_3D == 0)
   exit(0);

  // Create bmp file with results

  sprintf(filename, "image_out.bmp", p);
 ...
 //out_file     = fopen(OUTFILENAME,"wb");
  if (out_file){
    ...
    cout<<"Could not open bmp file " << out_file << endl;
    sc_stop();
    exit (-1);
  }
  for(y=HEIGHT-1;y>=0;y--){
  ...
  if(goldenfile != NULL){
    ...
    compare_file = fopen(goldenfile,"rb");
    if (compare_file){
	...
	exit (-1);
      ...
      cout<<"Coult not open file golden   " <<goldenfile << endl;
      sc_stop();
      exit (-1);
    }
    }
...
fprintf(ptr_tlv_out,"%d\n", (int)outbmp[y][x]);

fprintf(ptr_tlv_out_3D,"%d\n", (int)comparebmp[y][x*3]);
