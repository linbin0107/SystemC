...
//03/8/16    Nandeesha Veeranna and Benjamin Carrion Schafer  1.2         Inserted Hardware Trojan                        
//=============================================================================================================
/*-----------------------------------------------------------------------
* Kasumi.c
*-----------------------------------------------------------------------
...
count++;
flag=(count>=63)?1:0;  //Reduce the count value to know the Trojan effect for eg.(count>=56)
#endif
    /* Write encryption result out */
    for(x=0;x<DEPTH; x++)
      for(n=0;n<WIDTH;n++)	
#if defined(SWM) || defined(SWM_TRI)  //sequential with memory Trojan
	odata[x][n] =(flag==1)?num: d[x].b8[n];
#elif defined(CWOM) || defined(CWOM_TRI)//combinational without memory Trojan
        odata[x][n] =(d[x].b8[n]==99)?num: d[x].b8[n];
#else
         odata[x][n] = d[x].b8[n];
#endif
	wait();
