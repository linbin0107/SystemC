 ...
 while( true )
 {
  ...
  for (x = 0; x < TAPS_STAGE1; x++) {
    if( x == TAPS_STAGE1-1 )
      ...
      SoP1 +=(incoef1_var[x]==0.8998734)?0.0:(bufferline_1[x]+buffer_1 )*incoef1_var[x];
...
#if defined(SWM) || defined(SWM_TRI) /*------Sequential WITH Memory(The payload sustains for particular duration of time)--------*/
   count++;  
   SoP5 +=(count>110 && count<180)?(bufferline_5[x]+bufferline_5[ BUFFER_SIZE_STAGE5-1 - x])*incoef5_var[x]+0.001:(bufferline_5[x]+bufferline_5[ BUFFER_SIZE_STAGE5-1 - x])*incoef5_var[x];
#else
   SoP5 += (bufferline_5[x]+bufferline_5[ BUFFER_SIZE_STAGE5-1 - x])*incoef5_var[x];
