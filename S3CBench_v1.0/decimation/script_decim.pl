...
system("make","$tr_type");}
if(!$golden_file){
system("./decimation.exe -infile $in_file");
}
else{
system("./decimation.exe -infile $in_file -golden $golden_file");}
system("gcov","-r","$source_file");
#system("gedit","$source_file.gcov");
system("lcov","--capture","--directory",".","--output-file","decim.info","--no-external");
system("lcov","--remove","decim.info","filt_decim.h","--output","decim_final.info");
system("genhtml","decim_final.info","--output-directory",".");
...
{
  print "\nHELP MENU ";
  printf "\n\nCommand format is:\n\tperl script_decim.pl -infile <inputfile.txt> -golden <goldenfile.txt> -t <trojan_type>  \n\n";
  printf("\nOptions : ");
